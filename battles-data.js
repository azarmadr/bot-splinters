...
async function getBattles(player = '',bd,minRank=0) {
...
_battles.fromUsers = (players,{depth=2,minRank,blackSet=new Set(),fn='',cl=27}={})=>new Promise(res=>{
  ...
  Promise.resolve(_arr.chunk(ul,cl).reduce(
    (memo,ul_chunk)=>memo.then(bd=>
      Promise.all(ul_chunk.map(u=>getBattles(u,bd,minRank))).then(()=>bd)
