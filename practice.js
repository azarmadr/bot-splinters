    ...
    for (let browser; !browser?.process(); ) {
        try {
            browser = await puppeteer.launch({
                ...
                args: [
                    ...(args.PPTR_USER_DATA_DIR
                        ? [
                              `--user-data-dir=${args.PPTR_USER_DATA_DIR[0]}_${id}`,
                          ]
                        : []),
            ...
            await browser
                ...
                .overridePermissions('https://splinterlands.com/', [
                    'notifications',
                ]);
...
const outstanding_match = (u) =>
    Promise.race([
        ...
        return /Too many requests. Please wait a few minutes and try again./.test(
            x,
        )
            ? sleep(8e3).then(R.always(1))
            : x;
    });
...
async function getBattleId(acc, page) {
    let id;
    await sleep(1e2);
    for (; !id; await sleep(9e2))
        await outstanding_match(acc)
            .then((x) => (id = x?.id))
            .catch(log);
...
(async () => {
    ...
    let [[sPage], [tPage]] = await Promise.all([
        sBrowser.pages(),
        tBrowser.pages(),
    ]);
    ...
    for (let user; ; users.push(user)) {
        ...
        for (const opp of users) {
            ...
            const [B] = await Promise.all([
                sSM.battle('Challenge', opp.account, {
                    rating_level: 4,
                    allowed_cards: 'all',
                }),
            ...
            opp.cards ??= await tSM
                .cards(opp.account)
                .then(cards2Obj(opp.account))
                .catch(
                    (e) =>
                        log(
                            e,
                            'cards collection api didnt respond. Did you use oppname? avoid email!',
                        ) ?? {},
                );
            ...
            await [
                ...
                (memo, x) =>
                    memo.then((_) =>
                        startBotPlayMatch(...x)
                            .then(() => console.log(`Team submitted`))
                            .catch(async (e) => {
                                log(
                                    e,
            ...
            await Promise.all(
                [tPage, sPage].map((page, i) =>
                    page
                        .evaluate('SM.CurrentView.data')
