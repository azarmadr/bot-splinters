...
/* const cardBox = c => {
  c == null ?:
  const [id, lvl] = Array.isArray(c)?c:[c];
  const card = _card[id-1];
  return {
    isMon : cardBox(card.type == 'Monster'?[id, lvl]:null),
  }
}*/
...
const xtractRules = ruleset=>{
  ...
  let {attr_r, card_r} = ruleset.split('|').reduce((rules,cr)=>{
    secondary.includes(cr)?(rules.card_r=cr):rules.attr_r.push(cr);
    return rules},{attr_r:[]})
