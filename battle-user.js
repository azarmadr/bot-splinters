...
const battles = (player) => getBattleHistory(player)
  ...
  .then(() => { return new Promise((res,rej) => {
    fs.readFile(`./data/${process.env.ACCOUNT}_history.json`, 'utf8', (err, data) => {
      if (err) {
        console.log(`Error reading file from disk: ${err}`); rej(err)
      } else {
        battlesList = data ? [...battlesList, ...JSON.parse(data)] : battlesList;
      }
      battlesList = uniqueListByKey(battlesList.filter(x => x != undefined), "battle_queue_id")//.filter(x => x.player_rating_final > median(min_rating) - 729);
      //console.log(battlesList.length,'min_rating', median(min_rating), min_rating)
      console.log(battlesList.length)
      fs.writeFile(`data/${process.env.ACCOUNT}_history.json`, JSON.stringify(battlesList), function (err) {
        if (err) {
